...
public class TeamMatchHistoryScrubber implements IScrubber<CSLTeamMatchHistory> {
	...
	private final Logger LOGGER = Logger
			.getLogger(TeamMatchHistoryScrubber.class.getName());
	...
	public CSLTeamMatchHistory scrub() {
		LOGGER.fine(MessageFormat.format(
				"Team: <{0}> - Finding Match History IDs", teamID));
		CSLTeamMatchHistory matchHistory = new CSLTeamMatchHistory(teamID);
		try {
			csldoc = Jsoup.connect(URLResources.TEAM_URL + teamID)
		...
		for (Element rowGroup : rowGroups) {
			...
			// Do not add bye or future matches
			if (matchId.equals("#"))
				continue;

			matchHistory.addMatchId(matchId);
			LOGGER.fine(MessageFormat.format(
					"Team: <{0}> MatchId:<{1}> -Adding ID", teamID));
		}
		return matchHistory;
	}
