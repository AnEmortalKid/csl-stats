...
public class MatchHistoryScrubber implements
	...
	private void parse2v2RowHistory(Element ulGroup,
		...
		/*
		 * Evaluate players to see if they're equal, and if so, invalidate the
		 * match (we're not gonna count it) cause someone missreported /* stuff
		 */
		CSLPlayer p1 = players.get(0);
		CSLPlayer p2 = players.get(1);
		if (p1.equals(p2)) {
			System.out
					.println("2's team contained the same player twice, this match doesn't count.");
			return;
		}
